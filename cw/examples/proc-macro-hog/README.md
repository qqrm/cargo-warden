...
Set `WARDEN_EXAMPLE_EXPECT_WARNING=1` when building (for example, by running `WARDEN_FAKE_SANDBOX=1 cargo run --bin cargo-warden -- run -- env WARDEN_EXAMPLE_EXPECT_WARNING=1 cargo build -p warden-proc-macro-hog`) to surface the warning above.
