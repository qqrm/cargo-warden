...
// NOTE:
// `warden-bpf-core` is primarily built for the eBPF target. The unit tests in
// `src/tests.rs` exercise eBPF-only symbols (maps + LSM/cgroup entrypoints).
// Those symbols do not exist on non-BPF targets, so we only compile the test
// module when building for the BPF target.
