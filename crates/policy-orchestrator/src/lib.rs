...
mod tests {
    ...
    fn collect_policy_status_reports_workspace_source() {
        ...
        let status = collect_policy_status(&metadata, &[], None).unwrap();
    ...
    fn collect_policy_status_includes_cli_policy_and_override() {
        ...
        let metadata = fetch_metadata();
        let status = collect_policy_status(&metadata, &[cli_arg], Some(Mode::Enforce)).unwrap();
...
"#,
        ...
        let metadata = fetch_metadata();
        let isolation = configure_isolation(&metadata, &allow, &paths, None).unwrap();
    ...
    fn setup_isolation_defaults_to_empty_policy() {
        ...
        let metadata = fetch_metadata();
        let isolation = configure_isolation(&metadata, &[], &[], None).unwrap();
    ...
    fn setup_isolation_uses_cli_allow_when_no_file() {
        ...
        let metadata = fetch_metadata();
        let isolation = configure_isolation(&metadata, &allow, &[], None).unwrap();
...
"#,
        ...
        let metadata = fetch_metadata();
        let isolation = configure_isolation(&metadata, &[], &[], Some(Mode::Observe)).unwrap();
    ...
    fn setup_isolation_reports_duplicate_cli_allow() {
        ...
        let metadata = fetch_metadata();
        let Err(err) = configure_isolation(&metadata, &allow, &[], None) else {
